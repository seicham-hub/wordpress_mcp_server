...
server.tool("edit_post", {
    id: z.string(),
    title: z.string().optional(),
    content: z.string().optional(),
    status: z.string().optional(),
    categories: z.array(z.string()).optional(),
}, async ({ id, title, content, status, categories }) => {
    const body = {};
    if (title)
        body.title = title;
    if (content)
    ...
    if (status)
        body.status = status;
    if (categories)
        body.categories = categories.map(Number);
...
server.tool("list_posts", {}, async () => {
    ...
    const text = posts.map((post) => `ID: ${post.id}, タイトル: ${post.title.rendered}`).join("\n");
...
server.tool("list_categories", {}, async () => {
    ...
    const text = categories.map((cat) => `ID: ${cat.id}, 名前: ${cat.name}`).join("\n");
    return { content: [{ type: "text", text }] };
});
server.tool("edit_category", {
    id: z.string(),
    name: z.string(),
}, async ({ id, name }) => {
...
server.tool("create_category", {
    ...
    if (parent)
        body.parent = parseInt(parent, 10);
    const response = await wpFetch("/wp-json/wp/v2/categories", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body)
